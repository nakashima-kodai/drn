...
def show_loaded_image(image_A, image_B, nrow, normalize=False):
    grid_image = torch.cat((image_A, image_B), dim=0)
    grid_img = torchvision.utils.make_grid(grid_image, nrow=nrow, padding=1, normalize=normalize)
...
def show_label(label, nrow, normalize=False):
    grid_img = torchvision.utils.make_grid(label, nrow=nrow, padding=1, normalize=normalize)
    grid_img = grid_img.numpy().transpose((1, 2, 0))
...
def save_image(opt, epoch, saved_image):
    saved_image = torchvision.utils.make_grid(saved_image.cpu(), nrow=opt.batch_size, padding=1, normalize=False)
